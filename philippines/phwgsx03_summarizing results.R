...
source(paste0(path_gains_repo,"/philippines/phwgsx01_fitting models.R"))
source(paste0(path_gains_repo,"/philippines/phwgsx02_contrast matrices.R"))



(sex_stratified <- bind_rows(clean_mi_contrasts(models_bmi_ea,link = "lm",model_matrix = contrast_matrix_bmi_ea,vcov_type = "naive") %>% 
            mutate(Outcome = "BMI EA"),
          clean_mi_contrasts(models_bmi_ya,link = "lm",model_matrix = contrast_matrix_bmi_ya,vcov_type = "naive") %>% 
          ...
          clean_mi_contrasts(models_stress_ya,link = "lm",model_matrix = contrast_matrix_stress_ya,vcov_type = "naive") %>% 
  ...
  write.csv(.,paste0(path_dissertation,"/aim 2/working/cohorts gains/philippines/sex stratified_conditionals.csv"),row.names = FALSE)
