...
source(paste0(path_gains_repo,"/philippines/phwgnr02_fitting models with ipaw.R"))
...
bind_rows(clean_mi_conditionalregression(models_bmi_ea,link = "lmer identity") %>% 
  ...
  display_results_ph() %>% 
  arrange(iv) %>% 
  write.csv(.,paste0(path_dissertation,"/aim 2/working/cohorts gains/philippines/results_conditionals ipaw.csv"),row.names = FALSE)
