...
bind_rows(b9_df,
  ...
  export2xls(.,file=paste0(path_dissertation,"/aim 2/working/cohorts gains/table 1 revised compareGroups.xlsx"))
...
bind_rows(b9_df,
  ...
  mutate(bmi_ya = case_when(pregnant_ya == 1 ~ NA_real_,
         ...
         bmi_ea = case_when(pregnant_ea == 1 ~ NA_real_,
                            TRUE ~ bmi_ea)) %>% 
  mutate_at(vars(male,gtatole,full,black,
                 pregnant_ya,married_ea, married_ya,
                 white),~as.character(.)) %>% 
  tableby(site ~ chbirtho + male + gtatole + full +
            black + eduyr + formal_ya + pregnant_ya + children_ya + rural_ya + 
            formal_ea + pregnant_ea + children_ea + rural_ea + 
            married_ya + married_ea +
            wealth_child + wealth_teen + wealth_ya + wealth_ea +
  ...
  summary(.,text=TRUE) %>% 
  xlsx::write.xlsx(.,paste0(path_dissertation,"/aim 2/working/cohorts gains/table 1 revised arsenal.xlsx"),sheetName = "arsenal table1")
