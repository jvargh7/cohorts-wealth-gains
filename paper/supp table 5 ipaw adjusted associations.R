...
b9_df <- read_csv(paste0(path_dissertation,"/aim 2/working/cohorts gains/brazil 1993/results_conditionals ipaw.csv")) %>% 
  mutate(site = "Brazil")  %>% 
  rename_all(~str_replace(.," (EA|YA)",""))
gt_df <- read_csv(paste0(path_dissertation,"/aim 2/working/cohorts gains/guatemala/results_conditionals ipaw.csv")) %>% 
  mutate(site = "Guatemala") %>% 
  dplyr::select(-one_of("BMI YA")) %>% 
  rename_all(~str_replace(.," (EA|YA)",""))
ph_df <- read_csv(paste0(path_dissertation,"/aim 2/working/cohorts gains/philippines/results_conditionals ipaw.csv")) %>% 
  mutate(site = "Philippines") %>% 
  dplyr::select(-one_of("BMI YA","STRESS (z-scores) YA")) %>% 
  rename_all(~str_replace(.," (EA|YA)",""))
sa_df <- read_csv(paste0(path_dissertation,"/aim 2/working/cohorts gains/south africa/results_conditionals ipaw.csv"))%>% 
...
coefs_df %>% 
  ...
  write.csv(.,paste0(path_dissertation,"/aim 2/working/cohorts gains/supplementary table 5.csv"),row.names = FALSE)
