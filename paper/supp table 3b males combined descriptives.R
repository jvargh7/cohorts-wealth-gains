b9_df <- readRDS(paste0(path_dissertation,"/aim 2/working/cohorts gains/brazil 1993/absolute_df.RDS")) %>% 
  dplyr::filter(!is.na(bmi_ya)|!is.na(iq_ya)|!is.na(srq_ya)|!is.na(wellbeing_ya),male==1) %>% 
  mutate(site = "Brazil") %>% 
...
gt_df <- readRDS(paste0(path_dissertation,"/aim 2/working/cohorts gains/guatemala/absolute_df.RDS")) %>% 
  dplyr::filter(!is.na(bmi_ea)|!is.na(ravens_ea),male==1) %>% 
...
ph_df <- readRDS(paste0(path_dissertation,"/aim 2/working/cohorts gains/philippines/absolute_df.RDS")) %>% 
  dplyr::filter(!is.na(bmi_ea)|!is.na(ravens_ea)|!is.na(srq_ea)|!is.na(happiness_ea),male==1) %>% 
  mutate(site = "Philippines") %>% 
  rename(
    srq_ya = stress_ya)

sa_df <- readRDS(paste0(path_dissertation,"/aim 2/working/cohorts gains/south africa/absolute_df.RDS")) %>% 
  dplyr::filter(!is.na(bmi_ya)|!is.na(ravens_ea)|!is.na(srq_ea)|!is.na(happiness_ea),male==1) %>% 
...
bind_rows(b9_df,
  ...
  export2xls(.,file=paste0(path_dissertation,"/aim 2/working/cohorts gains/supplementary table 3B revised compareGroups.xlsx"))
...
bind_rows(b9_df,
  ...
  summary(.,text=TRUE) %>% 
  xlsx::write.xlsx(.,paste0(path_dissertation,"/aim 2/working/cohorts gains/supplementary table 3B revised arsenal.xlsx"),sheetName = "arsenal table1")
