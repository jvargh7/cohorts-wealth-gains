...
source(paste0(path_gains_repo,"/guatemala/gtwg05_fitting models.R"))
...
bind_rows(clean_mi_conditionalregression(models_bmi_ea,link = "lmer identity") %>% 
  ...
  arrange(iv) %>% 
  write.csv(.,paste0(path_dissertation,"/aim 2/working/cohorts gains/guatemala/results_conditionals.csv"),row.names = FALSE)
