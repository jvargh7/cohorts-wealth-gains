...
source(paste0(path_gains_repo,"/guatemala/gtwgnr02_fitting models with ipaw.R"))
source(paste0(path_gains_repo,"/guatemala/gtwgaux02_display results.R"))
...
bind_rows(clean_mi_conditionalregression(models_bmi_ea,link = "lmer identity") %>% 
  ...
  display_results_gtml() %>%
  arrange(iv) %>% 
  write.csv(.,paste0(path_dissertation,"/aim 2/working/cohorts gains/guatemala/results_conditionals ipaw.csv"),row.names = FALSE)
...
bind_rows(clean_mi_conditionalregression(models_bmi_ea,link = "lmer identity") %>% 
  ...
  dplyr::filter(iv %in% c("wealth_child","cwealth_teen","cwealth_ya","cwealth_ea",
                          "eduyr","rural_ya1","rural_ea1","formal_ya1","formal_ea1")) %>% 
  display_results_gtml(.,type="output") %>% 
