...
source(paste0(path_gains_repo,"/guatemala/gtcw01_fitting models for conditional wealth as outcome.R"))
...
bind_rows(clean_mi_conditionalregression(models_teen,link = "lmer identity") %>% 
  ...
  arrange(iv) %>% 
  write.csv(.,paste0(path_dissertation,"/aim 2/working/cohorts gains/guatemala/results_cwealth associations.csv"),row.names = FALSE)
