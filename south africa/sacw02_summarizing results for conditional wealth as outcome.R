...
source(paste0(path_gains_repo,"/south africa/sacw01_fitting models for conditional wealth as outcome.R"))
...
bind_rows(clean_mi_conditionalregression(models_teen,link = "lmer identity") %>% 
  ...
  display_results_sa() %>% 
  arrange(iv) %>% 
  write.csv(.,paste0(path_dissertation,"/aim 2/working/cohorts gains/south africa/results_cwealth associations.csv"),row.names = FALSE)
