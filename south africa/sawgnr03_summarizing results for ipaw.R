...
source(paste0(path_gains_repo,"/south africa/sawgnr02_fitting models with ipaw.R"))
...
bind_rows(clean_mi_conditionalregression(models_bmi_ya,link = "lmer identity") %>% 
  ...
  display_results_sa() %>% 
  arrange(iv) %>% 
  write.csv(.,paste0(path_dissertation,"/aim 2/working/cohorts gains/south africa/results_conditionals ipaw.csv"),row.names = FALSE)
