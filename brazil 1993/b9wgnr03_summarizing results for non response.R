...
source(paste0(path_gains_repo,"/brazil 1993/b9wgnr02_fitting models with ipaw.R"))
...
bind_rows(clean_mi_conditionalregression(models_bmi_ya,link = "lmer identity") %>% 
  ...
  display_results_b9() %>% 
  arrange(iv) %>% 
  write.csv(.,paste0(path_dissertation,"/aim 2/working/cohorts gains/brazil 1993/results_conditionals ipaw.csv"),row.names = FALSE)
